...
namespace BANK_SYSTEM {
    ...
    void AddAdminToFile() {
        ...
        admin.Password = "1234";
        admin.Permissions = enMainMenuePermissions::eAll;
        admin.MarkForDelete = false;

        if (MyFile.is_open()) {
            AdminLine = ConvertUserRecordToLine(admin);
            MyFile << AdminLine << endl;
            MyFile.close();
        }
    ...
    int main()
    {
        if (LoadUserInfo("Admin", "1234") == false) {
            AddAdminToFile();
        }
